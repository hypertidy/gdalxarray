...
def test_open_applies_options_thread_local(backend, synthetic_geotiff, recorder):
    backend.open_dataset(synthetic_geotiff, multidim=False, config_options=CFG)
...
def test_reads_reapply_options(backend, synthetic_geotiff, recorder):
    ds = backend.open_dataset(synthetic_geotiff, multidim=False, config_options=CFG)
    n_open = len(recorder)
...
def test_values_identical_with_and_without(backend, synthetic_geotiff):
    ...
    np.testing.assert_array_equal(plain["band_data"].values, opted["band_data"].values)
